...
local Groups = require("loona.groups")
...
function M.extras()
  local Extra = require("loona.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/loona/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/loona/config.lua", "\n(--@class loona%.Config.-\n})")
